...
func (bindingService *BindingService) GetBinding(id uint) (binding v2ray.Binding, err error) {
	err = global.GVA_DB.Debug().Where("id = ?", id).Preload("User").Preload("Server").First(&binding).Error
	return
}
...
func (bindingService *BindingService) GetBindingInfoList(info v2rayReq.BindingSearch) (list []v2ray.Binding, total int64, err error) {
	...
	err = db.Debug().Limit(limit).Offset(offset).Preload("User").Preload("Server").Find(&bindings).Error
...
// ReportBinding 上报绑定
func (bindingService *BindingService) ReportBinding(binding *v2ray.Binding) error {
	// 获取所有该服务器的绑定
	bindings, err := bindingService.GetBindingsByServerID(binding.ServerID)
	if err != nil {
		return err
	}
	...
	config := v2ray.NewXRayConfig(binding.Server.Port)
	config.InboundConfigs[0].Settings.Clients = settingsCli
	data, err := json.Marshal(config)
	if err != nil {
		return err
	}
	binding.Server.Config = data
	req, resp := fasthttp.AcquireRequest(), fasthttp.AcquireResponse()
	req.Header.SetMethod("POST")
	req.SetBody(data)
	req.SetRequestURI(fmt.Sprintf("http://%s:%d/conf/update", binding.Server.Ip, global.GVA_CONFIG.STAT_PORT))
