...
func (b *BaseApi) DeleteUser(c *gin.Context) {
	...
	if reqId.ID == 1 {
		response.FailWithMessage("删除失败, admin无法删除", c)
		return
	}
	count, err := bindingService.CountBindingByUserID(reqId.ID)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	if count > 0 {
		response.FailWithMessage("用户存在绑定关系", c)
		return
	}
...
func (b *BaseApi) GetAllUser(c *gin.Context) {
	...
	response.OkWithDetailed(gin.H{"users": users}, "获取成功", c)
}
