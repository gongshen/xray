...
var serverService = service.ServiceGroupApp.V2rayAdminServiceGroup.ServerService
...
// @Param data body v2ray_admin.Server true "创建Server"
...
// @accept application/json
// @Produce application/json
// @Param data body v2ray_admin.Server true "删除Server"
...
// @accept application/json
// @Produce application/json
// @Param data body v2ray_admin.Server true "更新Server"
...
func (serverApi *ServerApi) RestartXray(c *gin.Context) {
	var server v2ray.Server
	err := c.ShouldBindJSON(&server)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
	...
	if err = bindingService.ReportBinding(&server); err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	// 更新server的config字段
	if err = serverService.UpdateServerConfig(server.ID, server.Config); err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
